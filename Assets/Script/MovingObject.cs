...
public abstract class MovingObject : MonoBehaviour
{
    ...
    // Move는 x 방향, y 방향, RaycastHit2D에 대해 충돌을 확인하기 위한 매개변수를 취함.
    protected bool Move(int xDir, int yDir, out RaycastHit2D hit)
    {
        ...
        // Ray를 사용할 때 자신과 부딪히지 않기 위해 boxCollider 비활성화
        boxCollider.enabled = false;

        // 시작지점에서 끝지점의 라인을 가져와 blockingLayer과 충돌검사
    ...
    {
        // 이동할 남은 거리를 계산 ( 현재위치 - end ) 
        // sqrMagnitude : 벡터 길이 제곱
